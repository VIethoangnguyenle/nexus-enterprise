...
### Step 1.5: Detect Module vs Standalone (MANDATORY for NGAC)

Determine if this screen is:
- **A) Module inside existing app** — a screen within the NGAC workspace
- **B) Standalone page** — login, landing page, onboarding, error page

**If Module (most common case):**
Prepend this to the enhanced prompt:

```
APP SHELL CONTEXT (DO NOT OVERRIDE):
This screen is a MODULE inside an existing enterprise workspace app (like Lark/Feishu).
The following are ALREADY PROVIDED by the app shell — do NOT generate them:
- Left sidebar with navigation (Chat, Drive, Assets, Approvals, Contacts)
- Top bar with workspace name and notifications
- Bottom navigation bar (mobile)

Generate ONLY the main content area that sits to the right of the sidebar.
The content area should match the density and layout patterns of existing modules:
- List views use dense table rows (32px height, 12-16px padding)
- Filtering uses horizontal Tabs, not sidebar categories
- Detail views use a right-side panel (400px desktop, full-screen mobile)
- Empty states are centered with icon + title + description
```

**If Standalone:** No constraint block needed — design the full page.
