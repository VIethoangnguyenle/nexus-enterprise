...
export const Route = createFileRoute('/_assets/$assetId')({ component: AssetDetail })
...
function AssetDetail() {
  ...
  const timelineItems = history.map((h, i) => ({
    ...
    body: (
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        {h.comment && <Text variant="caption" muted className="ml-2">— {h.comment}</Text>}
      </div>
    ),
  }))
  ...
  return (
    <div className="animate-fade-in">
      ...
      {transition.error && (
        <div className="bg-danger-bg text-danger px-4 py-2 rounded-[var(--radius-sm)] text-sm mb-4">
              ...
              <>
                <div className="flex flex-wrap gap-2 mb-3">
                  {transData!.transitions.map(t => (
                    ...
                    >
                      {transition.isPending ? <Spinner size="sm" /> : <>{t.action} → <span className="text-accent">{t.to}</span></>}
                    </Button>
                  ))}
                </div>
