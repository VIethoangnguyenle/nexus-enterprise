...
2. **Số call site vỡ khi gộp variant là 5, không phải 7.** `variant="error"` chỉ có 1 chỗ thật trên `Button`; 2 chỗ còn lại thuộc `AlertBanner` (kiểu `AlertVariant` riêng) và 1 là comment.
