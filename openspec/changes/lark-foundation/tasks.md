...
- [ ] 12.2 Run `docker-compose up` and verify all services healthy
- [ ] 12.3 End-to-end test: create asset type → create asset → request → approve → assign → return → dispose
- [ ] 12.4 End-to-end test: asset events → Kafka → notifications appear in messaging
- [ ] 12.5 End-to-end test: thread creation, replies, entity linking
- [ ] 12.6 NGAC verification: verify permission inheritance (category → type → asset), prohibitions, cross-workspace isolation
